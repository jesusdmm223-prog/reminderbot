...
### Clave Generada
- **Tipo:** RSA 4096-bit
- **Nombre:** `id_rsa_contabo_pc_trabajo`
- **Ubicación:** `~/.ssh/id_rsa_contabo_pc_trabajo` (privada)
- **Ubicación pública:** `~/.ssh/id_rsa_contabo_pc_trabajo.pub`
- **Fingerprint:** `SHA256:ZBe0BUvG98SqLqSXtI3aYxqRq9RBySmd018Ivg3ILwc`
- **Comentario:** `pc-trabajo-contabo`
...
# PC Trabajo - Agente Financiero
Host contabo
    HostName 213.199.42.218
    User root
    IdentityFile ~/.ssh/id_rsa_contabo_pc_trabajo
...
ssh -i ~/.ssh/id_rsa_contabo_pc_trabajo root@213.199.42.218
```
...
# Local (en este PC)
chmod 700 ~/.ssh
chmod 600 ~/.ssh/id_rsa_contabo_pc_trabajo
chmod 644 ~/.ssh/id_rsa_contabo_pc_trabajo.pub
...
- Ambos PCs pueden conectarse simultáneamente sin problemas
...
ssh -o PasswordAuthentication=yes root@213.199.42.218 "cat ~/.ssh/authorized_keys | grep pc-trabajo"

# 2. Verificar permisos locales
ls -la ~/.ssh/id_rsa_contabo_pc_trabajo
...
ssh-keygen -t rsa -b 4096 -f ~/.ssh/id_rsa_contabo_pc_trabajo_new -C "pc-trabajo-contabo-new"

# 2. Copiar al servidor
ssh-copy-id -i ~/.ssh/id_rsa_contabo_pc_trabajo_new.pub root@213.199.42.218
...
1. **No compartir la clave privada** (`id_rsa_contabo_pc_trabajo`)
...
5. **La clave del otro PC** NO se ve afectada por esta configuración
